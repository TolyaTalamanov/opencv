...
def add(g_in1, g_in2, dtype):
    ...
    return cv.gapi_op('custom.add', custom_add_meta, g_in1, g_in2, dtype).getGMat()
...
def split3(g_in):
    ...
    op = cv.gapi_op('custom.split3', custom_split3_meta, g_in)
...
def mean(g_in):
    ...
    op = cv.gapi_op('custom.mean', custom_mean_meta, g_in)
...
def addC(g_in, g_sc, dtype):
    ...
    op = cv.gapi_op('custom.addC', custom_addC_meta, g_in, g_sc, dtype)
    return op.getGMat()
...
def size(g_in):
    ...
    op = cv.gapi_op('custom.size', custom_size_meta, g_in)
...
def sizeR(g_rect):
    ...
    op = cv.gapi_op('custom.sizeR', custom_sizeR_meta, g_rect)
    return op.getGOpaque(cv.gapi.CV_SIZE)
...
def boundingRect(g_array):
    ...
    op = cv.gapi_op('custom.boundingRect', custom_boundingRect_meta, g_array)
...
def goodFeaturesToTrack(g_in, max_corners, quality_lvl,
    ...
    op = cv.gapi_op('custom.goodFeaturesToTrack', custom_goodFeaturesToTrack_meta, g_in,
...
class gapi_sample_pipelines(NewOpenCVTests):
    ...
    def test_custom_size(self):
        ...
        pkg = cv.kernels((custom_size, 'org.opencv.streaming.size'))
        actual = comp.apply(cv.gin(in_mat), args=cv.compile_args(pkg))
    ...
    def test_custom_goodFeaturesToTrack(self):
        ...
        pkg = cv.kernels((custom_goodFeaturesToTrack, 'org.opencv.imgproc.feature.goodFeaturesToTrack'))
        actual = comp.apply(cv.gin(in_mat), args=cv.compile_args(pkg))
    ...
    def test_custom_addC(self):
        ...
        pkg = cv.kernels((custom_addC, 'org.opencv.core.math.addC'))
    ...
    def test_custom_sizeR(self):
        ...
        pkg = cv.kernels((custom_sizeR, 'org.opencv.streaming.sizeR'))
    ...
    def test_custom_boundingRect(self):
        ...
        pkg = cv.kernels((custom_boundingRect, 'org.opencv.imgproc.shape.boundingRectVector32S'))
    ...
    def test_custom_op_add(self):
        ...
        pkg = cv.kernels((custom_add, 'custom.add'))
        actual = comp.apply(cv.gin(in_mat1, in_mat2), args=cv.compile_args(pkg))
    ...
    def test_custom_op_split3(self):
        ...
        pkg = cv.kernels((custom_split3, 'custom.split3'))
    ...
    def test_custom_op_mean(self):
        ...
        pkg    = cv.kernels((custom_mean, 'custom.mean'))
        actual = comp.apply(cv.gin(in_mat), args=cv.compile_args(pkg))
    ...
    def test_custom_op_addC(self):
        ...
        pkg = cv.kernels((custom_addC, 'custom.addC'))
        actual = comp.apply(cv.gin(in_mat, sc), args=cv.compile_args(pkg))
    ...
    def test_custom_op_size(self):
        ...
        pkg = cv.kernels((custom_size, 'custom.size'))
        actual = comp.apply(cv.gin(in_mat), args=cv.compile_args(pkg))
    ...
    def test_custom_op_sizeR(self):
        ...
        pkg = cv.kernels((custom_sizeR, 'custom.sizeR'))
        actual = comp.apply(cv.gin(roi), args=cv.compile_args(pkg))
    ...
    def test_custom_op_boundingRect(self):
        ...
        pkg = cv.kernels((custom_boundingRect, 'custom.boundingRect'))
        actual = comp.apply(cv.gin(points), args=cv.compile_args(pkg))
    ...
    def test_custom_op_goodFeaturesToTrack(self):
        ...
        comp = cv.GComputation(cv.GIn(g_in), cv.GOut(g_out))
        pkg = cv.kernels((custom_goodFeaturesToTrack, 'custom.goodFeaturesToTrack'))
        actual = comp.apply(cv.gin(in_mat), args=cv.compile_args(pkg))
